            ...
            if n_patchcores == 1:
                ...
                for i in range(n_patchcores):
                    ...
                    patchcore_instance.load_from_path(
                        ...
                        device=device,
                        nn_method=nn_method,
                        #prepend="Ensemble-{}-{}_".format(i + 1, n_patchcores), # FIXME修改了这里
                    )
                    loaded_patchcores.append(patchcore_instance)
...
if __name__ == "__main__":
    ...
    # Set True to run without typing command-line arguments.
    USE_PRESET_ARGS = False

    # Edit this list directly when USE_PRESET_ARGS=True.
    PRESET_ARGS = [
        "--gpu", "0",
        "--seed", "0",
        "--save_segmentation_images",
        "evaluated_results/IM224_WR50_bottle_with_imgs",
        "patch_core_loader",
    ...
    # 将曾经运行的条件，写入log
    if USE_PRESET_ARGS:
        run_args = PRESET_ARGS
        LOGGER.info("Preset arguments: {}".format(" ".join(run_args)))
        run_source = "preset"
    else:
        run_args = sys.argv[1:]
        LOGGER.info("Command line arguments: {}".format(" ".join(sys.argv)))
        run_source = "cli"
    # Keep a lightweight history of previously used arguments.
    with open("run_args_history.log", "a", encoding="utf-8") as f:
        f.write("[{}] {}\n".format(run_source, " ".join(run_args)))
